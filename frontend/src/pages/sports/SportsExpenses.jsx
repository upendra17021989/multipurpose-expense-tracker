...
const initialForm = { sportsEventId: '', expenseDate: today, category: '', amount: '', paymentMode: 'CASH', vendorName: '', utr: '', chequeNumber: '', description: '', remarks: '', status: 'DRAFT' }
...
export const SportsExpenses = () => {
  ...
  const visibleExpenses = useMemo(() => {
    ...
    return expenses
      ...
      .filter((expense) => !query || [expense.category, expense.vendorName, expense.description, expense.eventName].filter(Boolean).some((value) => String(value).toLowerCase().includes(query)))
  }, [expenses, filters])
  ...
  const submit = async (event) => {
    ...
    try {
      await sportsAPI.createExpense({ ...form, sportsEventId: form.sportsEventId ? Number(form.sportsEventId) : null, amount: Number(form.amount), utr: form.utr || null, chequeNumber: form.chequeNumber || null })
      setForm(initialForm)
  ...
  return (
    <Shell title="Sports Expenses" eyebrow="Sports module">
      ...
      <form className="inline-form" onSubmit={submit}>
        <select value={form.sportsEventId} onChange={(event) => setForm({ ...form, sportsEventId: event.target.value })}>
          <option value="">No event</option>
          {events.map((sportsEvent) => <option key={sportsEvent.id} value={sportsEvent.id}>{sportsEvent.eventName}</option>)}
        </select>
        <input type="date" value={form.expenseDate} onChange={(event) => setForm({ ...form, expenseDate: event.target.value })} required />
        ...
        <select value={form.paymentMode} onChange={(event) => setForm({ ...form, paymentMode: event.target.value })}>{paymentModes.map((mode) => <option key={mode}>{mode}</option>)}</select>
        <input placeholder="Vendor" value={form.vendorName} onChange={(event) => setForm({ ...form, vendorName: event.target.value })} />
        <input placeholder="UTR" value={form.utr} onChange={(event) => setForm({ ...form, utr: event.target.value })} />
        <button className="primary" type="submit">Add Expense</button>
      </form>
      <section className="toolbar-panel flat-toolbar">
        <input placeholder="Search event, category, vendor" value={filters.search} onChange={(event) => setFilters({ ...filters, search: event.target.value })} />
      ...
      <div className="table-wrap">
        <table>
          <thead><tr><th>Date</th><th>Event</th><th>Category</th><th>Vendor</th><th>Payment</th><th>Status</th><th className="numeric">Amount</th><th>Actions</th></tr></thead>
          <tbody>
            {visibleExpenses.map((expense) => <tr key={expense.id}><td>{formatDate(expense.expenseDate)}</td><td>{expense.eventName || '-'}</td><td>{expense.category}</td><td>{expense.vendorName || '-'}</td><td>{expense.paymentMode}</td><td>{expense.status}</td><td className="numeric">{formatCurrency(expense.amount)}</td><td className="table-actions"><button className="danger" onClick={() => remove(expense.id)}>Delete</button></td></tr>)}
            {!loading && visibleExpenses.length === 0 && <tr><td colSpan="8" className="empty-state">No sports expenses found.</td></tr>}
          </tbody>
        </table>
      </div>
