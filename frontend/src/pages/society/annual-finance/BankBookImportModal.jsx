...
export const BankBookImportModal = ({ open, financialYear, flats, onClose, onImported }) => {
  ...
  const previewFile = async () => {
    if (!file) return toast.error('Select an Excel bank book')
    setWorking(true)
    try {
      const response = await societyAnnualCollectionAPI.previewBankBook(file, financialYear)
      setPreview(response.data)
    } catch (error) {
      toast.error(error.response?.data?.message || 'Unable to preview the bank book')
  ...
  const confirmImport = async () => {
    ...
    try {
      ...
      toast.success(`${response.data.created} bank book entries imported`)
      onImported()
      onClose()
    } catch (error) {
      toast.error(error.response?.data?.message || 'Unable to import the bank book')
    } finally { setWorking(false) }
  }
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={() => !working && onClose()}>
    <section className="expense-modal import-modal bank-book-import-modal" role="dialog" aria-modal="true" aria-labelledby="bank-book-import-title" onMouseDown={event => event.stopPropagation()}>
      <div className="expense-modal-header"><div><h2 id="bank-book-import-title">Import Bank Book</h2><p className="muted">Excel receipts will create maintenance collections for {financialYear}.</p></div><button className="modal-close" onClick={onClose} disabled={working}>×</button></div>
      <div className="expense-modal-form">
        <label>Bank book Excel<input type="file" accept=".xlsx,.xls" onChange={event => { setFile(event.target.files?.[0] || null); setPreview(null) }}/></label>
      ...
      {preview && <>
        ...
        <div className="table-wrap bank-book-preview-table"><table><thead><tr><th>Row</th><th>Date</th><th>Excel flat</th><th>Matched flat</th><th>Payer</th><th>Reference</th><th className="numeric">Amount</th><th>Status</th></tr></thead><tbody>
          {preview.rows.map((row, index) => <tr key={`${row.rowNumber}-${row.sourceReference}`}><td>{row.rowNumber}</td><td>{row.date ? formatDate(row.date) : '-'}</td><td>{row.flatText || '-'}</td><td><select aria-label={`Flat for Excel row ${row.rowNumber}`} value={row.flatId || ''} disabled={row.duplicate} onChange={event => selectFlat(index, event.target.value)}><option value="">Select flat</option>{flats.map(flat => <option key={flat.id} value={flat.id}>{flat.blockName}-{flat.flatNumber}</option>)}</select></td><td>{row.sourceName || '-'}</td><td>{row.referenceNumber || row.transactionId || '-'}</td><td className="numeric">{formatCurrency(row.debit)}</td><td>{row.duplicate ? <span className="status-pill cancelled">Duplicate</span> : row.errors?.length ? <span className="status-pill overdue">Invalid</span> : row.flatId ? <span className="status-pill paid">Ready</span> : <span className="status-pill pending">Select flat</span>}</td></tr>)}
        </tbody></table></div>
