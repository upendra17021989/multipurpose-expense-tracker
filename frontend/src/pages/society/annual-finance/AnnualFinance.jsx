...
export const AnnualFinance=()=>{
 ...
 return <Shell title="Annual Finance" eyebrow="Society module" actions={<div className="table-actions"><button onClick={()=>setModal('collection')}>Record Collection</button><button className="primary" onClick={()=>setModal('expense')}>Add Expense</button></div>}>
  ...
  {activeView==='collections'&&<div className="table-wrap"><table><thead><tr><th>Date</th><th>Type</th><th>Flat</th><th>Source</th><th>Mode</th><th>Reference</th><th className="numeric">Amount</th><th>Action</th></tr></thead><tbody>{rows.map(x=><tr key={x.id}><td>{formatDate(x.paymentDate)}</td><td>{x.collectionType}</td><td>{x.flatLabel||'-'}</td><td>{x.sourceName}</td><td>{x.paymentMode}</td><td>{x.referenceNumber||'-'}</td><td className="numeric">{formatCurrency(x.amount)}</td><td className="table-actions"><button onClick={()=>setModal({type:'collection',item:x})}>Edit</button><button className="danger" onClick={()=>remove(x.id)}>Delete</button></td></tr>)}{!rows.length&&<tr><td colSpan="8" className="empty-state">No collections recorded.</td></tr>}</tbody></table></div>}
  {activeView==='expenses'&&<><div className="table-wrap"><table><thead><tr><th>Date</th><th>Category</th><th>Vendor</th><th>Payment</th><th>Status</th><th className="numeric">Amount</th><th>Action</th></tr></thead><tbody>{visibleExpenses.map(x=><tr key={x.id}><td>{formatDate(x.expenseDate)}</td><td>{x.categoryName||'-'}</td><td>{x.vendorName||'-'}</td><td>{x.paymentMode}</td><td><span className={`status-pill ${String(x.status).toLowerCase()}`}>{x.status}</span></td><td className="numeric">{formatCurrency(x.amount)}</td><td><button onClick={()=>setModal({type:'expense',item:x})}>Edit</button></td></tr>)}{!annualExpenses.length&&<tr><td colSpan="7" className="empty-state">No expenses found.</td></tr>}</tbody></table></div>{pageCount>1&&<nav className="table-pagination"><button disabled={expensePage===1} onClick={()=>setExpensePage(x=>x-1)}>Previous</button><span>Page {expensePage} of {pageCount}</span><button disabled={expensePage===pageCount} onClick={()=>setExpensePage(x=>x+1)}>Next</button></nav>}</>}
  <CollectionModal open={modal==='collection'||modal?.type==='collection'} collection={modal?.item} financialYear={financialYear} flats={flats} onClose={()=>setModal(null)} onSaved={()=>{loadCollections();setActiveView('collections')}}/>
  <ExpenseModal open={modal==='expense'||modal?.type==='expense'} expense={modal?.item} financialYear={financialYear} onClose={()=>setModal(null)} onSaved={()=>{loadExpenses();setActiveView('expenses')}}/>
 </Shell>
}
