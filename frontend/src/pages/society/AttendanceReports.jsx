  ...
  return <Shell title="Attendance Reports" eyebrow="Society workforce">
    ...
    <div className="table-wrap">
      {daily ? <table>
        ...
        <tbody>{(report?.rows || []).map((row) => <tr key={row.attendanceId}>
          <td><strong>{row.workerName}</strong><br /><small>{row.workerType}</small></td>
        ...
        <tbody>{(report?.workers || []).map((row) => <tr key={row.workerKey}>
          <td><strong>{row.workerName}</strong><br /><small>{row.workerType}</small></td>
          <td>{row.agencyName || 'Direct'}</td><td>{row.shiftName}<br /><small>{row.postName}</small></td>
