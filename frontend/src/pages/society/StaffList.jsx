...
export const StaffList = () => {
  ...
  return <Shell title="Society Staff" eyebrow="Society module" actions={canWrite && <Link className="button-link" to="/society/staff/new">Add Staff</Link>}>
    ...
    <div className="table-wrap"><table><thead><tr><th>Name</th><th>Designation</th><th>Mobile</th><th>Joining Date</th><th className="numeric">Monthly Salary</th><th>Actions</th></tr></thead><tbody>
      {visible.map((s) => <tr key={s.id}><td>{s.staffName}</td><td>{s.designation}</td><td>{s.mobile || '-'}</td><td>{s.joiningDate ? formatDate(s.joiningDate) : '-'}</td><td className="numeric">{formatCurrency(s.monthlySalary)}</td><td className="table-actions">{canWrite ? <><button onClick={() => navigate(`/society/staff/${s.id}/edit`)}>Edit</button><button className="danger" onClick={() => remove(s.id)}>Remove</button></> : <span className="muted">View only</span>}</td></tr>)}
