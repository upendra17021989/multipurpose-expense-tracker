...
export const SocietyMemberDirectory = () => {
  ...
  const financialLedger = (rows, ledgerKey) => {
    ...
    return <>
    <div className="table-wrap society-flat-financial-table"><table><thead><tr><th>Date</th><th>Mode</th><th>Payer</th><th>Reference</th><th>UTR / Transaction ID</th><th>Narration</th><th>Details</th><th className="numeric">Amount</th></tr></thead><tbody>
      {visibleRows.map((row) => <tr key={row.id}><td>{formatDate(row.paymentDate)}</td><td>{row.paymentMode}</td><td>{row.sourceName || '-'}</td><td>{row.referenceNumber || '-'}</td><td>{row.transactionId || row.settlementId || '-'}</td><td className="society-ledger-narration">{row.narration || '-'}</td><td>{row.remarks || '-'}</td><td className="numeric">{formatCurrency(row.amount)}</td></tr>)}
      {!rows.length && <tr><td colSpan="8" className="empty-state">No cash or bank book transactions for this flat in {financialYear}.</td></tr>}
    </tbody></table></div>
