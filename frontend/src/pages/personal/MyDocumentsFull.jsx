  ...
  return <Shell title="My Documents" eyebrow="Individual module">
    ...
    <section className="panel">
      ...
      {loading ? <p className="muted document-state">Loading documents...</p> : documents.length === 0 ? <p className="muted document-state">No documents match your filters.</p> : <div className="document-list">{documents.map((document) => {
        const [status, tone] = statusFor(document.expiryDate)
        return <article className="document-card" key={document.id}><div className="document-card-main"><div><h3>{document.title}</h3><p title={document.originalFileName}>{labels[document.category]} · {document.originalFileName}</p>{document.sharedWithMe && <span className="shared-document-label">Shared with me · Read only</span>}</div><span className={`document-status ${tone}`}>{status}</span></div>
          ...
          <div className="document-card-controls">{document.tags && <p className="document-tags" title={document.tags}>{document.tags}</p>}<div className="document-actions"><button onClick={() => preview(document)}>View</button><button onClick={() => download(document)}>Download</button>{!document.sharedWithMe && <><button onClick={() => share(document)}>Share</button><button onClick={() => { setEditing(document); setFormOpen(true); window.scrollTo({ top: 0, behavior: 'smooth' }) }}>Edit</button><button className="danger document-delete-button" onClick={() => remove(document)}>Delete</button></>}</div></div></article>
      })}</div>}
