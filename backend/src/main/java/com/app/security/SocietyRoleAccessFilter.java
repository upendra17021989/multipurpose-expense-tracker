...
public class SocietyRoleAccessFilter extends OncePerRequestFilter {
    ...
    private boolean staffSupervisorPermitted(String method, String path) {
        if ("GET".equals(method) || "HEAD".equals(method)) {
            return path.startsWith("/society/daily-operations") || path.matches("/attachments(?:/\\d+/download)?") || path.equals("/expenses/categories") || path.matches("/society/notifications(?:/unread-count)?") || path.matches("/society/complaints(?:/\\d+)?") || path.matches("/society/flats(?:/\\d+)?") || path.matches("/society/staff(?:/\\d+)?") || path.matches("/society/vendors(?:/\\d+)?") || path.matches("/society/agencies(?:/\\d+)?(?:/workers(?:/\\d+)?)?") || path.matches("/society/shifts(?:/\\d+)?") || path.matches("/society/roster-assignments(?:/\\d+)?") || path.matches("/society/attendance(?:/shortages|/reports/(?:daily|monthly))?") || path.equals("/society/attendance-workflow/status") || path.matches("/society/work-orders(?:/\\d+(?:/expense-requests)?)?");
        }
