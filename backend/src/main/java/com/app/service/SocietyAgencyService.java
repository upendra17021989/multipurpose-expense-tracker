...
public class SocietyAgencyService {
 ...
 @Transactional(readOnly=true) public List<AgencyDto> list(Long accountId){ society(accountId); List<SocietyAgency> rows=agencies.findByAccountIdAndActiveTrueOrderByNameAsc(accountId);if(rows.isEmpty())return List.of();Map<Long,List<SocietyAgencyWorker>> byAgency=workers.findByAccountIdAndAgencyIdInAndActiveTrueOrderByWorkerNameAsc(accountId,rows.stream().map(SocietyAgency::getId).toList()).stream().collect(java.util.stream.Collectors.groupingBy(w->w.getAgency().getId()));return rows.stream().map(a->agencyDto(a,byAgency.getOrDefault(a.getId(),List.of()))).toList(); }
 ...
 private AgencyDto map(SocietyAgency a,boolean includeWorkers){return agencyDto(a,includeWorkers?workers.findByAccountIdAndAgencyIdAndActiveTrueOrderByWorkerNameAsc(a.getAccount().getId(),a.getId()):List.of());}
 private AgencyDto agencyDto(SocietyAgency a,List<SocietyAgencyWorker> agencyWorkers){return AgencyDto.builder().id(a.getId()).name(a.getName()).serviceCategory(a.getServiceCategory()).contactPerson(a.getContactPerson()).phone(a.getPhone()).email(a.getEmail()).contractStart(a.getContractStart()).contractEnd(a.getContractEnd()).requiredHeadcount(a.getRequiredHeadcount()).workers(agencyWorkers.stream().map(this::mapWorker).toList()).build();}
