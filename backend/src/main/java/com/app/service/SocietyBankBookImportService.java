...
public class SocietyBankBookImportService {
    ...
    public SocietyBankBookImportDtos.Preview preview(Long accountId, String financialYear, MultipartFile file) {
        ...
        try (Workbook workbook = WorkbookFactory.create(file.getInputStream())) {
            ...
            requireHeaders(columns, "date", "type", "flat no.", "particulars", "txn id / cheque no.", "reference number", "debit", "credit", "balance");
            ...
            for (int i=header.getRowNum()+1;i<=sheet.getLastRowNum();i++) {
                ...
                if (type.isBlank() || particulars.toLowerCase(Locale.ROOT).contains("opening balance") || !normalizedType.contains("bankreciept") && !normalizedType.contains("bankreceipt")) { skipped++; continue; }
        ...
        } catch (Exception ex) { if(ex instanceof ValidationException validation)throw validation; throw new ValidationException("Unable to read bank book: "+ex.getMessage()); }
    }
    ...
    public SocietyBankBookImportDtos.Result confirm(Long accountId, Long userId, SocietyBankBookImportDtos.ConfirmRequest request) {
        ...
        for(SocietyBankBookImportDtos.Row row:request.getRows()) {
            ...
            SocietyAnnualCollection collection=collectionRepository.save(SocietyAnnualCollection.builder().account(account).flat(flat).financialYear(request.getFinancialYear())
                    ...
                    .paymentMode(PaymentMode.BANK).referenceNumber(clean(row.getReferenceNumber(),null)).transactionId(clean(row.getTransactionId(),null)).settlementId(clean(row.getSettlementId(),null))
    ...
    private SocietyBankBookImportDtos.Row parse(Row row,Map<String,Integer> c,DataFormatter f,FormulaEvaluator e,List<Flat> flats,Long accountId,String financialYear){
        ...
        String sourceReference=!transactionId.isBlank()?"BANKBOOK:TXN:"+transactionId.trim().toUpperCase(Locale.ROOT):!settlement.isBlank()?"BANKBOOK:SETTLEMENT:"+settlement.trim().toUpperCase(Locale.ROOT):"BANKBOOK:"+date+":"+normalize(flatText)+":"+debit+":"+reference;
        ...
        return SocietyBankBookImportDtos.Row.builder().rowNumber(row.getRowNum()+1).date(date).type(text(row,c.get("type"),f,e)).flatText(flatText)
                ...
                .transactionId(transactionId).referenceNumber(reference).voucherNumber(voucher).settlementId(settlement).debit(debit).credit(credit).balance(balance)
    ...
    private String sourceName(String particulars){if(particulars==null)return null;String first=particulars.split("\\R",2)[0].trim();return first.replaceFirst("(?i)^By\\s+","").trim();}
    private String remarks(SocietyBankBookImportDtos.Row r){return String.join(" | ",List.of("Bank book import",optional("Voucher",r.getVoucherNumber()))).replaceAll(" \\| $","");}
    private String optional(String label,String value){return value==null||value.isBlank()?"":label+": "+value;}
    private Row findHeader(Sheet s,DataFormatter f,FormulaEvaluator e){for(Row row:s){Map<String,Integer>h=headers(row,f,e);if(h.containsKey("date")&&h.containsKey("flat no.")&&h.containsKey("debit"))return row;}throw new ValidationException("Could not find the bank book header row");}
    private Map<String,Integer> headers(Row r,DataFormatter f,FormulaEvaluator e){Map<String,Integer>m=new HashMap<>();for(Cell cell:r)m.put(f.formatCellValue(cell,e).trim().toLowerCase(Locale.ROOT),cell.getColumnIndex());return m;}
    ...
    private Account societyAccount(Long id){Account a=accountRepository.findById(id).orElseThrow(()->new ValidationException("Account not found"));if(a.getAccountType()!=AccountType.SOCIETY)throw new ValidationException("Bank book import is available only for society accounts");return a;}
